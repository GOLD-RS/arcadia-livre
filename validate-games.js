...
const modules={click:'js/games-click.js',memory:'js/games-memory.js',snake:'games-snake.js',math:'games-math.js',stack:'games-stack.js',dodge:'games-dodge.js',breaker:'games-breaker.js',pop:'js/games-pop.js',mole:'js/games-mole.js',color:'js/games-color.js',runner:'js/games-runner.js'};
...
for(const [key,page] of Object.entries(expected)){
 ...
 if(modules[key]){
  if(!html.includes(modules[key])) throw new Error(`${page}: módulo específico ausente ${modules[key]}`);
  if(['js/games-pop.js','js/games-mole.js','js/games-color.js','js/games-runner.js'].some(x=>x!==modules[key]&&html.includes(x))) throw new Error(`${page}: módulo clássico indevido`);
...
for(const f of Object.values(modules)) if(!fs.existsSync(root+'/'+f)) throw new Error(`Módulo ausente: ${f}`);
...
for(const f of Object.values(modules)){const src=fs.readFileSync(root+'/'+f,'utf8');if(src.includes('pagePaused=false')) throw new Error(`${f}: pagePaused fixo`);if(!src.includes('lifecycle')) throw new Error(`${f}: sem lifecycle`);if(!src.includes('registerCleanup(')) throw new Error(`${f}: sem registerCleanup`);if(src.includes('setCleanup')) throw new Error(`${f}: usa setCleanup legado`)}
...
for(const f of [...new Set(Object.values(modules))]){
