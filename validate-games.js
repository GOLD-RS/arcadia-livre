...
for(const f of Object.values(classic)){const src=fs.readFileSync(root+'/'+f,'utf8');if(src.includes('pagePaused=false')) throw new Error(`${f}: pagePaused fixo`);if(!src.includes('lifecycle')) throw new Error(`${f}: sem lifecycle`)}
